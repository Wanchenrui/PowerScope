...
状态：T01 冻结供 T03/T04/T05 消费；新 Python 数据类尚未接入业务。线版本仍为 1，schema 版本独立为 1。依据 PC e2ce8ab、FW 3469da8eaeb2b5657381df49817e350f35964c15 的实际 `user/source/debug_monitor_core.c` 与同名头文件；不以 `mcu_debug_stub` 为证据。
...
请求有效载荷上限192、请求整帧206；响应整帧192、普通响应数据上限181。ReadMemory(01) 的 payload 恰为一个非零 u8 长度。当前实现允许1..192，182..192读取后在组帧处静默丢响应，这是缺陷，不是成功。**T03 proposed**：182/192 明确 NACK LENGTH；193现有即 LENGTH。PC立即采用1..181分块上限，不能等待固件修复。
...
设备控制0C、运行模式0D、清故障0E和Wave20..26现有实现；头文件枚举本身不能用于推导命令已支持。响应暂存槽只有1项，满时可能丢响应；超时是Unknown。流看门狗为3,000,000us且需流中GetInfo启用，不是功率停机保证。

## GetInfo：现有与 proposed 明确分离

现有空请求，94字节payload。PC `DebugService.parse_device_info` 逐长度读取可选字段，但当前仅检查44字节；T04应至少要求完整58字节稳定前缀。短扩展不得补零伪造能力。
...
构建完成manifest绑定build ID、上述输入记录、PC/FW commits、DevicePack版本、ELF/bin/所有控制静态库SHA256、工具链/ABI/链接选项及内存区间；manifest自身hash由PC本地记录，不能再塞回参与构建的输入形成循环。`artifacts_verified`仅由核验器在设备ID等于manifest ID且本地ELF/bin/hash一致时设置；不依据版本字符串、elf_crc=0或ELF符号存在设置。它不是设备认证。T00现存ELF只作现有构建物，不证明当前源码重新构建成功。
...
Golden检查见tests/test_contracts.py：固定十六进制输入与PC实际native编解码对照；它只证明PC构帧/CRC和边界helper，**不证明实际FW已修复**。T03必须将同样帧输入真实Dm_ProcessFrame路径并检查181/182/192/193响应行为；64位测试机使用窄内存访问接缝，禁止直接解引用MCU地址。
